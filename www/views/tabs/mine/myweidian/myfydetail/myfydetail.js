...
angular.module('App').controller('MyFydetailController',function($ionicScrollDelegate,$ionicModal,$timeout,$ionicHistory,$scope,$state,$stateParams,$http,$Factory,$ionicSlideBoxDelegate,$ionicPopover,$timeout,$ionicLoading,WechatService,Apphost){
	$timeout(function(){
		$('span.back-text').css('display','none');
	
		if($('#myfydetail .myfydetail').innerWidth()>375){
			$('#myfydetail .myfydetail').addClass('plus')
			$('#myfydetail .myfydetail').removeClass('myfydetail')
		}
	})
	...
	//把浮动框读取到作用域中
	// .fromTemplateUrl() 方法
	$ionicPopover.fromTemplateUrl('myfydetailpopover.html', {
	    scope: $scope
	...
	$scope.Base64=function(shareto){
		var mycans = document.getElementById("myfydetailmycans");
		    ...
		    $scope.params = {
		        scene: shareto,
			        ...
			        media: {
			       		...
			       		webpageUrl:Apphost.apphost+"/#/tabs/mine/myweidian/myfydetail/"+$stateParams.id+'/'+$stateParams.housetype// 分享的url
			    	}
			    }
		    };
