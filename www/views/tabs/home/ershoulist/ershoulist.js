...
angular.module('App').controller('ErshoulistController',function($ionicScrollDelegate,$ionicLoading,$ionicHistory,$state,$http,$Factory,$scope,$rootScope,$stateParams,$ionicPopover,$ionicPopup,$timeout,$ionicActionSheet){
	$timeout(function(){
...
//		$('.fyfabu').css('margin-top',$('.header').outerHeight()+40)
		...
		if($('#ershoulist .ershoulist').innerWidth()>375){
			$('#ershoulist .ershoulist').addClass('plus')
			$('#ershoulist .ershoulist').removeClass('ershoulist')
		}
	})
	...
	$scope.closeoption=function($event){
		$('#ershoulist .ershou .item-content').css('transform','translate3d(0px, 0px, 0px)')
	}
	...
	$scope.openchoose=function(index){
		...
		$('#ershoulist .pullchooselist').slideToggle('fast')
		$('#ershoulist .mask').toggle()
	}
	...
	$scope.onlyclosechoose=function(){
		$('#ershoulist .pullchooselist').css('display','none')
		$('#ershoulist .mask').css('display','none')
	}
