...
angular.module('App').controller('ChuzuSubController',function($ionicHistory,$ionicLoading,$timeout,$Factory,$http,$scope,$stateParams){
	setTimeout(function(){
		$('span.back-text').css('display','none');
		$('.chuzusub').css('margin-top',$('.header').outerHeight())
	})
...
//		OutsideHouseRentSubscription:localStorage.getItem('OutsideHouseRentSubscription').split(','),
...
//  }
	...
	//					console.log(resData.data.data)
						...
						for(item of $scope.userinfo.OutsideHouseRentSubscription){
						    if(item=='思明'){
   ...
   $scope.chk= function(){
		   	if($scope.devList[0].checked==false&&$scope.devList[1].checked==false&&$scope.devList[2].checked==false&&$scope.devList[3].checked==false&&$scope.devList[4].checked==false&&$scope.devList[5].checked==false&&$scope.devList[6].checked==false&&$scope.devList[7].checked==false){
				...
				$scope.userinfo.OutsideHouseRentSubscription=[];
				var obj=$('#chuzusub input[type="checkbox"]'); 
				//取到对象数组后，我们来循环检测它是不是被选中 
				for(var i=0; i<obj.length; i++){ 
					if(obj[i].checked){
						$scope.userinfo.OutsideHouseRentSubscription.push($scope.devList[i].text);
					} 
				} 
				...
				$http(req).then(function(resData){
					if(resData.data.status==0){
						localStorage.setItem('OutsideHouseRentSubscription',$scope.userinfo.OutsideHouseRentSubscription)
						$ionicLoading.show({
