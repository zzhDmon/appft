...
angular.module('App').controller('ChushouController',function($ionicScrollDelegate,$timeout,$http,$Factory,$rootScope,$scope,$stateParams,$ionicLoading,$ionicHistory){
	$scope.back=function(){
		$ionicHistory.goBack();
	}

	$scope.isSub=localStorage.getItem('OutsideHouseSubscription')
	...
	$scope.subScrollListen = function(){
		...
		if(scrollTop>300){
			$('#chushou .backtop').css('display','block')
		}else{
			$('#chushou .backtop').css('display','none')
		}
	}
	...
	function load(loadType,subtype) {
		$http.get($Factory.OutsideHouse.query.url,{params:{isSubs:subtype,pagenum:pagenum,pagesize:pagesize},cache:false}).then(function(resData) {
			//返回空数组，没有更多数据了
