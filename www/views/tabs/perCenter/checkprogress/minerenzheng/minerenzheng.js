...
angular.module('App').controller('MinerenzhengController',function($ionicLoading,$ionicHistory,$state,$http,$Factory,$scope,$rootScope,$stateParams,$ionicPopover,$ionicPopup,$timeout,$ionicActionSheet){
	$timeout(function(){
		$('span.back-text').css('display','none');
		
		if($('#minerenzheng .minerenzheng').innerWidth()>320){
			$('#minerenzheng .minerenzheng').addClass('plus')
//			$('#renzheng .renzheng').removeClass('renzheng')
	...
	$('#mineaddIdcard').on('click','.add',function(){
		$(this).next().click();
	});
	$('#mineaddIdcard').on('change','input',function(){
		var file = $(this)[0].files[0];
		$scope.idcardBase64(URL.createObjectURL(file),1)
	});
	
	 $('#minebusinesscard').on('click','.add',function(){
		$(this).next().click();
	});
	$('#minebusinesscard').on('change','input',function(){
		var file = $(this)[0].files[0];
	...
	$scope.idcardBase64=function(url,index){
		if(index==1){
			var mycans = document.getElementById("mineidcardcans");
			...
			var mycans = document.getElementById("minebusinesscardcans");
			var cxt = mycans.getContext("2d");
			var img = new Image();
			img.src = url;
			img.onload = function(){
